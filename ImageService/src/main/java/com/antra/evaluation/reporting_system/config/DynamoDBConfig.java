...
public class DynamoDBConfig {
    ...
    // Create the ImageFile table on startup if it doesn't exist (idempotent), so
    // local/dev and CI work without a manual provisioning step.
    @Bean
    public ApplicationRunner ensureImageFileTable(AmazonDynamoDB amazonDynamoDB, DynamoDBMapper mapper,
                                                  @Value("${app.dynamodb.auto-create-table:true}") boolean autoCreate) {
        return args -> {
            if (!autoCreate) {
                return;
            }
            CreateTableRequest request = mapper.generateCreateTableRequest(ImageFile.class)
